...
import { computePlacePreviewCandidates } from './catalogAdjust'
import type { WizardState } from './types'

interface PlacePreviewStepProps {
  state: WizardState
...
export default function PlacePreviewStep({ state, onFinish, onBack }: PlacePreviewStepProps) {
  const candidates = useMemo(() => computePlacePreviewCandidates(state), [state])
  ...
  function decide(item: ProgramItem, isLiked: boolean) {
    const nextLiked = isLiked ? [...liked, item.id] : liked
    const nextDisliked = !isLiked ? [...disliked, item.id] : disliked
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center px-4">
      ...
      >
        Helyszín-előnézet -- {index + 1}/{candidates.length}
      </motion.span>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              ...
              {current.priceRange && (
                <span className="font-montserrat text-sm font-semibold text-parisian-beige-700">
                  {current.priceRange}
                </span>
              )}
            </div>
            ...
            {current.description && (
              <p className="mb-4 font-montserrat text-sm leading-relaxed text-parisian-grey-600">
                {current.description}
              </p>
            )}
      ...
      <div className="mt-8 flex items-center gap-4">
        <motion.button
          ...
          className="flex h-14 w-14 items-center justify-center rounded-full border-2 border-parisian-grey-200 bg-white text-parisian-grey-500 shadow-sm transition-colors hover:border-french-red-300 hover:text-french-red-500"
          aria-label="Kihagyom"
        >
        ...
        <motion.button
          ...
          className="flex h-14 w-14 items-center justify-center rounded-full border-2 border-parisian-beige-300 bg-parisian-beige-50 text-parisian-beige-600 shadow-sm transition-colors hover:border-parisian-beige-400"
          aria-label="Ez tetszik"
        >
