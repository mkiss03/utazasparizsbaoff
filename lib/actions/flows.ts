...
export async function loadFlow(destinationSlug: string): Promise<LoadFlowResult> {
  // A flow-tábla (vagy akár a planner Supabase-kapcsolat) hiánya nem
  // szabad, hogy az egész szerkesztőt letörje -- ilyenkor az editor a
  // beépített seed flow-val indul, csak mentéskor derül ki a hiba.
  try {
    ...
    return { graph: null, error: error instanceof Error ? error.message : 'Ismeretlen hiba a flow betöltésekor' }
  }
}
...
export async function saveFlow(destinationSlug: string, graph: FlowGraph): Promise<SaveFlowResult> {
  try {
    ...
    return { success: false, error: error instanceof Error ? error.message : 'Ismeretlen hiba a flow mentésekor' }
  }
}
