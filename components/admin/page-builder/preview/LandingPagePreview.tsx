...
import { LandingPageSettings, DEFAULT_SECTION_ORDER } from '@/lib/types/landing-page'
...
const SECTION_LABELS: Record<string, string> = {
  ...
  walkingTours: 'Sétatúrák',
  louvreTour: 'Interaktív Túrák',
  flashcardsPromo: 'Flashcards',
  parisDistrictGuide: 'Kerületek',
  museumGuidePromo: 'Múzeum Guide',
...
function renderPreviewSection(key: string, settings: LandingPageSettings): React.ReactNode {
  switch (key) {
    case 'hero':
      return (
        <div
  ...
  return (
    <div ref={wrapperRef} className="w-full select-none">
      ...
      >
        ...
        >
          {(settings.sectionOrder || DEFAULT_SECTION_ORDER).map((sectionKey) => {
            const s = settings as any
            if (!s[sectionKey]?.visible) return null
            ...
            return (
              ...
              >
                {renderPreviewSection(sectionKey, settings)}
              </PreviewSection>
            )
          })}
        </div>
