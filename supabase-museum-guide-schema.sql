...
-- Auto-update updated_at trigger (DROP first to avoid "already exists" error)
...
DROP TRIGGER IF EXISTS update_museum_guide_artworks_updated_at ON museum_guide_artworks;
...
DROP POLICY IF EXISTS "Museum guide artworks are publicly readable" ON museum_guide_artworks;
DROP POLICY IF EXISTS "Admins can manage museum guide artworks" ON museum_guide_artworks;
...
-- (csak akkor szúrja be, ha a tábla üres)
-- ============================================

INSERT INTO museum_guide_artworks (title, artist, year, wing, floor, room, story, fun_fact, gradient, map_position_x, map_position_y, display_order)
SELECT * FROM (VALUES
(
  'Mona Lisa',
  'Leonardo da Vinci',
...
(
  ...
  65::NUMERIC, 38::NUMERIC, 6
)
) AS seed(title, artist, year, wing, floor, room, story, fun_fact, gradient, map_position_x, map_position_y, display_order)
WHERE NOT EXISTS (SELECT 1 FROM museum_guide_artworks LIMIT 1);
